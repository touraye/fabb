...
import { advocacy_images } from '@/constant';

function EmpowermentGallery() {
  ...
  return (
    <div className='w-full xl:pt-8 md:pt-3'>
        <main className="flex flex-col items-center justify-center py-6 m-auto">
            ...
            {advocacy_images.map((image, index) => (
                <div key={index} 
            ...
            <Lightbox
              ...
              close={() => setOpen(false)}
              index={index}
              slides={advocacy_images.map((img) => ({ src: img.src }))}
...
export default EmpowermentGallery